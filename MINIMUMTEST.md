...
minimumtest/
  ...
  profiles/
    ...
    iku/                 — 個体「iku」
      ...
      core/              — コアモジュール
        config.py        — 定数・パス・DualLogger・llm_cfg
...
1個体 = 1ディレクトリ。`profiles/_template/`をコピーしてseed.txtとsettings.jsonを編集するだけで新しい個体が作れる。コード（main.py, core/, tools/）も個体ごとに独立しており、self_modifyで改変しても他の個体に影響しない。

run.bat起動時にプロファイルを番号で選択。同じ仕組みから、seed（種）が違うだけで異なる存在が育つ。
...
#### リファクタリング（1ファイル→モジュール分割）

main.py（2821行）を18ファイルに分割。

- `main.py`（441行）: エントリポイント + メインループのみ
- `core/`（11ファイル）: config, state, llm, embedding, eval, parser, entropy, memory, prompt, controller
- `tools/`（7ファイル）: builtin, web, x_tools, elyth_tools, memory_tool, sandbox, __init__（TOOLS辞書）

循環依存を避けるため、TOOLS辞書やtool_namesは引数で渡す設計。

#### プロファイル化（個体管理）

全コード+データを`profiles/個体名/`に格納する構成に移行。

- **1個体 = 1ディレクトリ**: main.py, core/, tools/, seed.txt, state.json, pref.json, settings.json, memory/, sandbox/が全て個体ディレクトリ内に完結
- **_template**: 新規プロファイル作成用。コピーしてseed.txtを編集するだけ
- **self_modifyの安全性**: main.pyを改変しても自分のプロファイルのみに影響。他の個体は無傷
- **run.bat**: 起動時にプロファイル一覧を番号表示→選択
- **iku.txt → seed.txt**: ファイル名を一般化。ツール解放条件も`seed.txt`に変更
- **venvは共通**: minimumtest/.venvを全プロファイルで共有

#### その他の改善（Phase 11）

- **ツールチェーン実行**: propose複数ツール→1ツールずつexecuteに分解して順次実行。2つ目以降に前ツール結果を渡す
- **executeツール一覧制限**: 選択されたツールのみ表示（LLMの逸脱防止）
- **propose temperature=1.0 / execute temperature=0.4**: 提案は発散、実行は収束
- **ツールマーカーパーサー強化**: 閉じ引用符なし・出力切れ・丸括弧付きツール名のフォールバック
- **隠しファイル**: settings.json等をAIから非表示。pref.json/state.jsonはLevel 6で解放
- **Xセッション連動**: セッションなし時にX系ツールをallowed_toolsから除外。通知サマリーも連動停止
- **起動時Xログインチェック**: Level 3以上でセッションなしなら手動ログインを求める
- **LLM呼び出しtimeout**: 120s→300s
- **search_memoryソートバグ修正**: dict比較エラーを解消
- **Ctrl+C**: KeyboardInterruptでクリーン終了
...
### 将来検討: 行動一貫性指標（coherence）

intent-expect-resultの三者一致度を測る指標。E1-E3のペアワイズ評価とは別に、「意志のベクトルがブレていないか」を直接評価する。

**サプライズボーナスとの関係**: 直交する2軸。サプライズは「予想外の結果が新しい情報をもたらす」（情報理論的）。一貫性は「意志→予測→結果がブレてない」（実存的）。両立する。

```
              一貫性高い           一貫性低い
サプライズ高い  最高の学習           混乱の中の偶然
サプライズ低い  安定した遂行         迷走
```

**custom_drivesとの連携**: Level 6でcustom_drivesが解放されると、一貫性の「基準ベクトル」がintent-expect-resultの三者だけでなく、AIが自分で定義した動機（固有のベクトル）との整合性も測れるようになる。「やりたいこと(intent)が自分のdrive(custom_drives)と一致しているか」という上位の一貫性。

**導入タイミング**: custom_drives解放（Level 6到達）後が最適。それ以前は基準ベクトルがないため効果が薄い。

### 懸念: measured_entropyフィードバック率の初期過剰

measured_entropyは初期段階（ログ20件未満）でデータ不足のため全指標が「高エントロピー」と判定される。フィードバック率0.1だと毎tick +0.02程度conceptual_entropyを押し上げ、negentropyによる回復（0.01-0.04/サイクル）を上回る。結果として初期はentropyが高止まりしやすい。

**「最初は高エントロピーで、活動するほど下がる」という動き自体は設計思想として正しい。** ただし初期の「何をしてもentropyが下がらない」体験がLLMの行動パターンに悪影響を及ぼす可能性がある（何をしても無駄→wait傾向等）。

**対策候補**:
- フィードバック率を0.1→0.03に下げる
- ログ20件未満ではmeasuredフィードバックを無効にする
- measured_entropyの各指標にデータ不足時の補正を入れる

**現状**: 様子見。20サイクル以降でmeasuredが実状を反映し始めれば自然にバランスが取れる見込み。

### 将来検討: 行動前内省（reflect）

propose→select→executeの間に「本当にこれでいいか？」のreflectステップを挟む構想。

```
今:    propose → select → execute
将来:  propose → select → 【reflect】 → execute or re-select
```

**情報理論的な位置づけ**: Active Inferenceのexpected free energy（期待自由エネルギー）に相当。「この行動をしたらentropyがどうなるか」を事前に見積もり、低品質な選択を実行前に弾く。L2（知覚層）の範囲であり、L3（意志）への介入ではない。

**現状の代替手段**: intent-conditioned scoringが事後学習ベースで同様の役割を果たしている。「前にこのintent×toolでうまくいかなかった」をスコアに反映。reflectは事前予測、ICSは事後学習。

**導入しない理由（現時点）**:
- LLM呼び出しが1サイクル4回に（propose, reflect, execute, eval）。速度低下
- 9Bモデルの「この行動は良くないからやめよう」判断の精度が不明
- 新アーキテクチャ（measured_entropy, spiral_vector, LLM評価）の検証が先

**導入条件**: LLMの速度が十分（1回5秒以下）、かつICSだけでは防げない低品質行動が頻発する場合。
